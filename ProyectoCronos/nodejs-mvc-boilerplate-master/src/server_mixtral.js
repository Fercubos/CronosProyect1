...
app.post("/databases", async function (req, res) {
	...
	var promptNumberOfTasks =
		"Genera y dime cuantas tareas tiene el proyecto unicamente genera el numero de tareas nada mas, generalo en formato numerico sin otra palabra! del proyecto:"+ name + "!";
    ...
    //hacemos un regex para que la fecha sea en el formato correcto
    var regex = /^\d{4}-\d{2}-\d{2}$/;
    if (true) {
        // intentamos buscar la fecha en el texto
                var date = response_dueDate.match(/\d{4}-\d{2}-\d{2}/);
                if (date) {
                    response_dueDate = date[0];
                    console.log(response_dueDate);
                } else {
                    // si no encontramos la fecha, ponemos hoy como fecha de vencimiento
                    response_dueDate = DateTime.now().toFormat("yyyy-MM-dd");
                    console.log(response_dueDate);
                }
            }
	...
	for (let i = 2; i <= NumberOfTasks; i++) {
		...
		var response_dueDate = await DBsd(
			...
			"Genera y dime la fecha de vencimiento de la tarea numero " +
				i +
            " del proyecto en formato AAAA-MM-DD! UNICA Y EXCLUSIVAMENTE LA FECHA DE VENCIMIENTO DE LA TAREA GENERALO INVENTALO!"
		);
        if (true) {
            // intentamos buscar la fecha en el texto
                    var date = response_dueDate.match(/\d{4}-\d{2}-\d{2}/);
...
async function DBsd(response1, prompts) {
    ...
    const response = await axios.post(url_of_ngrok + '/v1/chat/completions', {
        ...
        messages: [
            ...
            { role: "user", content: prompts + userQuestion }
        ],
        temperature: 0.7,
        max_tokens: 200,
        stream: false
